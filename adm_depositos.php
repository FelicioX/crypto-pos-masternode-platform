						...
						<thead><tr><th>Investidor</th><th>Moeda</th><th>Quantidade</th><th>Data</th><th>Ação</th></tr></thead>
							<?php 
								...
								$result = mysqli_query($conect, $query); //resultado do select
									while ($row = mysqli_fetch_array($result)) {
										echo "<tr align='center'><td>" . $row['investidor'] . "</td><td>" . $row['moeda'] . "</td><td>" . $row['qnt'] . "</td>
										<td>" . date('d/m/Y - H:i:s', strtotime($row['data'])) . "</td><td>
										<button class='bt-process' data-toggle='modal' data-target='#myModal' onClick='detalhesDeposito(" . $row['id'] . ")'>Detalhes</button></td></tr>";
									}
